...
/*
    ...
    f - файлы
    d - дирректории
    h - показывать скрытые файлы, только если вызво идёт из php файла есть переменная
    ...
    if ($src) {
        ...
        if ($ican) {
            if (isset($_GET['e']) && $_GET['e']) {
                //Какие нужны расширения//Делятся запятой
                $e = preg_split('/,/', $_GET['e']);
            } else {
                $e = false;
            }
            ...
            if (isset($_GET['obj'])) {
                //Вернуть результат объектом, если возможно
                $isobj = (int) $_GET['obj'];
            } else {
                $isobj = false;
            }
            ...
            if ($sort) {
                ...
                for ($i = 0, $l = sizeof($filelist);$i < $l;++$i) {
                    ...
                    if ($sort == 'name') {
                        ...
                        $val = filesize($dir.infra_tofs($name));
                    } elseif ($sort == 'time') {
                        $val = filemtime($dir.infra_tofs($name));
                    }
                    $d['val'] = $val;
